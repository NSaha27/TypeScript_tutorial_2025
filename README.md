...
### 12. What are the differences between Array and Tuple datatypes?

1.  In array we can store same type of data only, while in Tuple we can store different types of data,
2.  An array can have a dynamic length, it can grow or shrink, but a Tuple can only have a fixed length of data which should be predefined.
3.  The order of the elements of an array can be changed, but for a Tuple the order of the elements must be fixed.
